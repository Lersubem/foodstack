...
- Domain-oriented namespaces:
  - FoodStack.Domain.Menu
  - FoodStack.Domain.Order
- Interface-driven services:
  - IMenuService with MenuServiceFile implementation.
  - IOrderService with OrderServiceFile implementation.
- File-based persistence using JSON files under a Data folder.

Frontend app

- Next.js App Router with TypeScript and strict typing.
- Main routes:
  - `/` – main menu + cart + ordering.
  - `/order` – order confirmation (from localStorage or backend).
- UI structured into:
  - Menu listing (grouped by menu).
  - Cart popup.
  - Theme switcher popup.
  - Global submit overlay for high-load indication.
...
- Custom ExceptionHandlingMiddleware wraps all requests:
  - Logs unhandled exceptions.
  - In development: returns the exception as plain text.
  - In production: returns a generic JSON `{ status, error, message }` without leaking stack traces.
- RequestSimulationMiddleware simulates high-load / degraded network behavior for POST /api calls (described again in Section 5).
...
Swagger is configured with OrderPlacementResultSchemaFilter to describe this as a oneOf union with discriminator "status". This makes it very clear to clients which structured payload they receive for each outcome.

---

## 4. Backend – Endpoints

All endpoints are served under the PathBase `/foodstack/service`, so the full path becomes `/foodstack/service/api/...`.
...
## 5. Backend – High-Load / Degraded Behavior Simulation
...
- Cart state handled via context and custom hook.
...
- `/order` (Order confirmation)
  ...
  - Includes “Place another order” button to navigate back to `/`.
...
## 7. Frontend – Extra Functionalities
...
- Displayed per cart item:
  - Trash icon (fully remove the item and its quantity).
  - Meal thumbnail
  - Name and menu name.
  - Inline quantity controls with `+` and `−` buttons.
- Custom scrollbar:
  - cartPopupBody is scrollable.
  - A custom scrollbar element is rendered alongside it.
...
- Each theme button:
  ...
  - Uses inline style with accent HSL variables so the button visually reflects the palette.
  - Shows three swatches (accent, highlight, background).
  - On click, applies the palette to the document and closes the popup.

- On app startup:

  - A default palette "Sunset" is applied automatically via a useEffect hook.
...
## 8. High Load & Resilience (End-to-End)

Backend and frontend are wired together to explicitly demonstrate resiliency under load.

Backend

- RequestSimulationMiddleware (Section 5) affects POST /api calls:
  - Randomly delays them.
  - Streams them slowly.
  - Or aborts mid-response.
...
- Submit overlay:
  ...
  - When an order is being placed:
    ...
    - A full-screen submitOverlay component is rendered

  - A separate state (showHighLoadMessage) is toggled by a timeout:
    - After a threshold, the message appears:
      - “We are experiencing high load. Please be patient while we process your order.”

- Top-level loading state:

  - The <main> root gets an isLoading class when either:
    - isSubmitting is true, or
    - menus are still loading.
  - This allows CSS to visually dim or otherwise indicate global loading.

- Retry behavior:

  - Order placement wraps fetch in a loop with a small maxAttempts count.
  - After exhausting attempts, the user gets a clear error message instead of a half-broken state.
...
- The app starts on a local port (e.g. http://localhost:5124) with PathBase `/foodstack/service`.
- Swagger (development only) is available at:

    http://localhost:5124/foodstack/service/swagger

Menu endpoints locally:

- GET http://localhost:5124/foodstack/service/api/menu
- GET http://localhost:5124/foodstack/service/api/menu/ids

Order endpoints locally:

- POST http://localhost:5124/foodstack/service/api/orders
- GET  http://localhost:5124/foodstack/service/api/orders/{orderID}
- GET  http://localhost:5124/foodstack/service/api/orders/by-request/{requestID}
...
- Contents of `out/` can be deployed to the `/foodstack` virtual directory on IIS (or any static host).
- Backend is deployed separately as an ASP.NET Core app, mounted under `/foodstack/service`.
...
## 12. Limitations & Possible Extensions

Deliberate simplifications:

- File-based JSON storage instead of a relational database.
- No authentication, user accounts, or sessions.
- No payments, promotions, or multi-tenant separation.
- No explicit CDN configuration (but menu endpoints are suitable for being cached).

Potential next steps:

- Replace MenuServiceFile and OrderServiceFile with database-backed implementations.
- Add caching headers and ETag support for menus so that CDNs can cache aggressively.
- Add metrics and distributed tracing (OpenTelemetry, Prometheus).
- Add pagination or filtering for large menus.
- Persist theme choice across visits (cookie or backend profile).
- Add advanced rate limiting and request priority for peak hours.
